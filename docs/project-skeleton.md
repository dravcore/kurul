...
outcome: deleting a board would *fail* rather than cascade. Owned children cascade:

```
Workspace → Board → Column, Task → Comment, Activity, TaskAssignee, TaskLabel
```

Each of those relations is declared `onDelete: Cascade`. References to *shared* rows do not
