...
**Updated:** 2026-08-18 — the Consequences section's "no index was added for the sweep's own predicates" no longer holds for three of the four tables: migration `20260814180000_retention_sweep_indexes` adds `Session_expiresAt_idx`, `Verification_expiresAt_idx` and `UsagePing_createdAt_idx`, after measuring the sweep at production-like volume found each on a sequential scan of the whole table (issue #187). `Activity.createdAt` and `Notification.readAt` are deliberately left alone — both already reach an index through a different leading column despite this ADR's prediction otherwise, so the trade-off below still holds for those two. This is the ADR's own "revisit only if a sweep is observed to outlast its window" clause working as designed: the decision was revisited on measurement, not assumption, and only where measurement asked for it.
