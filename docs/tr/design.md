...
route olarak sunar. **Koyu**: altı check de PASS, worst adjacent CVD ΔE 8.4.
...
| ---------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Kendi surface'i üzerinde body metni      | 4.5:1                                    | `app/globals.contrast.test.ts`: §3'teki her metin token'ı altı gerçek surface'e karşı (canvas, column, kart, popover, hover adımı, signature tint); boundary token'ları aynı altısını 3:1'de tutar, `--border` muaftır (state taşımayan dekoratif hairline); kalan tek istisna `opacity-50` üzerinden disabled metindir, WCAG onu zaten muaf tutar ve gate yine de 3:1'e tutar |
...
| Forced colors, high contrast | Bir surface adımı veya bir tint üzerine kurulu her state, border tabanlı bir ikizini taşır. `forced-colors: active` altında: seçili kart bir `Highlight` outline alır, column'un drop target'ı kendi tint'i yerine inset bir `Highlight` outline alır, ve highlighted bir menü satırı kendi tint'i yerine `Highlight` / `HighlightText` boyar. `prefers-contrast: more` altında: `--border`, ikinci bir palet açmak yerine `--border-strong`'un değerini alır. |
