...
**NestJS + TypeScript**, ORM olarak **Prisma 7**, **PostgreSQL 18** ve **Redis 8**.
...
- **Prisma 7 ücretsiz bir upgrade değil ve kırıcı değişiklikleri iskeleti şekillendiriyor — iskelet sırasında keşfedilmek yerine baştan planlanıyor:**
  ...
  - Generator'ın **`output` yolu zorunlu** ve `node_modules` dışında bir yere işaret etmeli. `packages/shared-types`, DTO tiplerini generate edilen modellerden türetiyor, dolayısıyla bu yolun hem `apps/api`'den hem de pnpm workspace içindeki `packages/shared-types`'tan çözümlenebilmesi gerekiyor.
