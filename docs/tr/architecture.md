...
| `openapi`      | OpenAPI dokümanını derlenmiş uygulamadan kurar, `API_DOCS_ENABLED` arkasında `/docs` ve `/openapi.json` üzerinden sunar ve commit'lenmiş `apps/api/openapi.json`'a karşı `pnpm openapi:check` drift kapısını besler. Sürümü `apps/api/package.json`'dan gelir, böylece doküman ile sürüm çelişemez                                                                    |
