...
| `INSTANCE_ADMIN_EMAILS`               | _(blank)_                                                           | Comma-separated addresses allowed to read the instance-wide [activation funnel](#activation-funnel-and-telemetry) **and to execute an erasure request on somebody else's account** (`DELETE /instance/users/:userId`, [ADR 0026](decisions/0026-account-deletion-anonymisation.md)). **Blank means nobody**, including the account that owns every workspace |
