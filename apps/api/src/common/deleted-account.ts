 ...
 * Auth's `session.cookieCache` answers `auth.api.getSession` from a signed cookie for five
 * minutes **without consulting the database** (`auth.ts`, and
 ...
 * to **every authenticated request in the product** to do it — for a five-minute window after a
