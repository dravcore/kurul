...
export function CommentBody({ body, className }: CommentBodyProps): React.ReactElement {
  ...
  return (
    <p className={cn('whitespace-pre-wrap text-read text-foreground', className)}>
      {tokens.map((token, index) => {
        ...
        return (
          <span
            ...
            className="inline-flex items-center rounded-[var(--radius-sm)] bg-signature-subtle px-1 py-0.5 text-small font-strong text-foreground"
